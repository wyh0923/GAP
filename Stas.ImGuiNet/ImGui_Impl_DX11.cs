...
namespace Stas.ImGuiNet {
    ...
    public class ImGui_Impl_DX11 {
        ...
        //https://gitter.im/ImGui-NET/Lobby?at=5e61d51f8e04426dd026cd3a
        void addFonts() {
            var io = ImGui.GetIO();
            ...
            //        ImGui.GetIO().Fonts.AddFontFromMemoryTTF((IntPtr)ptr, fontData.Length, size);
            //    }
            //}
            unsafe void loadIcon(string path, float size) {
                var fontData = File.ReadAllBytes(path);
                fixed(byte* ptr = fontData) {
                    ...
                    var ranges = new ushort[] { 14, 24, 0 };// { (ushort)FontAwesome5.IconMin, (ushort)FontAwesome5.IconMax, 0 };
                    fixed(ushort* rangesPtr = ranges) {
                        ImGui.GetIO().Fonts.AddFontFromMemoryTTF((IntPtr)ptr, fontData.Length, size, config, (IntPtr)rangesPtr);
                    }
            ...
            //loadFont("data/fonts/roboto_regular.ttf", 16);
            loadIcon("data/fonts/fa-regular-400.otf", 16);
            loadIcon("data/fonts/fa-solid-400.otf", 16);

            io.Fonts.Build();
        }
        public void CreateFontsTexture() {
            var io = ImGui.GetIO();
            //var font_ptr =io.Fonts.AddFontFromFileTTF(@"C:\log\Fontin-SmallCaps.ttf", 16);
