...
partial class DrawMain {
    ...
    public void DrawAHKSettings() {
        ...
        if ( ImGui.CollapsingHeader("Setup information - study pls")){
            DrawInfoOnQuad(info_one);
            ImGui.Separator();
            DrawInfoOnQuad(info_two);
        }
        ...
        ImGuiExt.ToolTip("Creates a default profile with an example " +
            "\nof using several basic flasks (silver, life & mana)");

        ImGui.SameLine();
        if (ImGui.Button("Save")) {
            ui.ahk.sett.Save();
        }
        ImGuiExt.ToolTip("Duplicates automatic saving on changes.");
