...
public class Profile
{
    ...
    public void DrawSettings()
    {
        if (ImGui.BeginTabBar("Profile Rules", ImGuiTabBarFlags.AutoSelectNewTabs | ImGuiTabBarFlags.Reorderable))
        {
            ...
            for (var i = 0; i < this.Rules.Count; i++)
            {
                ...
                if (!currRule.Enabled)
                {
                    ImGui.PushStyleColor(ImGuiCol.Text, ImGuiExt.Color(255, 75, 0, 255));
                }

                var isSelected = ImGui.BeginTabItem($"{currRule.Name}###Rule{i}", ref shouldNotDelete,  
    ...
    {
        ImGui.SetNextWindowPos(DrawMain.curr_info_rect.GetCenter());
