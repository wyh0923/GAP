...
public class Rule {
    ...
    public static Rule[] CreateDefaultRules() {
        ...
        rules[0] = new($"QuickSilver ["+ ui.hot_keys.use_flask_in_slot1.Key + "]");
        rules[0].Enabled = false;
        rules[0].Key = ui.hot_keys.use_flask_in_slot1.Key;
        ...
        rules[1] = new("Mana ["+ ui.hot_keys.use_flask_in_slot2.Key + "]");
        rules[1].Enabled = true;
        rules[1].Key = ui.hot_keys.use_flask_in_slot2.Key;
        rules[1].conditions.Add(new VitalsCondition(OperatorType.LESS_THAN, VitalType.MANA_PERCENT, 20));
        rules[1].conditions.Add(new FlaskChargesCondition(OperatorType.BIGGER_THAN, 2, 6));
        rules[1].conditions.Add(new FlaskEffectCondition(2));

        rules[2] = new($"Life ["+ ui.hot_keys.use_flask_in_slot3.Key + "]");
        rules[2].Enabled = true;
        rules[2].Key = ui.hot_keys.use_flask_in_slot3.Key;
        rules[2].conditions.Add(new VitalsCondition(OperatorType.LESS_THAN, VitalType.LIFE_PERCENT, 65));
        ...
        rules[3] = new($"Life ["+ ui.hot_keys.use_flask_in_slot4.Key + "]");
        rules[3].Enabled = true;
        rules[3].Key = ui.hot_keys.use_flask_in_slot4.Key;
        rules[3].conditions.Add(new VitalsCondition(OperatorType.LESS_THAN, VitalType.LIFE_PERCENT, 65));
        rules[3].conditions.Add(new FlaskChargesCondition(OperatorType.BIGGER_THAN, 4, 6));
        rules[3].conditions.Add(new FlaskEffectCondition(4));

        //rules[4] = new($"Life [F8]");
        //rules[4].Enabled = true;
        //rules[4].Key = Keys.F8;
        //rules[4].conditions.Add(new VitalsCondition(OperatorType.LESS_THAN, VitalType.LIFE_PERCENT, 65));
        //rules[4].conditions.Add(new FlaskChargesCondition(OperatorType.BIGGER_THAN, 5, 6));
        //rules[4].conditions.Add(new FlaskEffectCondition(5));
        return rules;
    }
    ...
    public void Execute(Action<string> logger) {
        if (this.Enabled && this.Evaluate()) {
            Keyboard.KeyPress(this.Key, tName + ".Execute");
            Thread.Sleep(R.Next(90, 200));
