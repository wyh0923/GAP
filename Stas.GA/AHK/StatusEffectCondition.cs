...
public class StatusEffectCondition : ICondition
{
    ...
    private void ToImGui(bool expand = true)
    {
        ...
        if (this.@operator != OperatorType.CONTAINS && this.@operator != OperatorType.NOT_CONTAINS)
        {
            ...
            ImGui.Text("Me has (de)buff");
            ImGui.SameLine();
            ImGui.SetNextItemWidth(60);
            ImGui.InputText("with", ref this.buffId, 200);
            ImGui.SameLine();
            ImGui.SetNextItemWidth(100);
            ImGuiExt.EnumComboBox("##comparison", ref this.@operator, SupportedOperatorTypes);
            ImGui.SameLine();
            ImGui.SetNextItemWidth(40);
            ImGui.InputFloat("##threshold", ref this.threshold);
            ImGui.SameLine();
            ImGui.SetNextItemWidth(100);
            ImGuiExt.EnumComboBox("##checkType", ref this.checkType);
            ImGuiExt.ToolTip($"What to compare. {StatusEffectCheckType.DURATION_PERCENT} ranges from " +
