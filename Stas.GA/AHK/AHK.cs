...
public class AHK {
    ...
    public bool ShouldExecutePlugin() {
        ...
        if (ui.b_town) {
            this.debugMessage = "I'm in town.";
            return false;
        }

        if (!sett.ShouldRunInHideout && ui.b_home) {
            this.debugMessage = "I'm in hideout.";
            return false;
        }

        if (ui.b_I_died) {
            this.debugMessage = "I'm dead.";
            return false;
        }

        if (ui.me.GetComp<Buffs>(out var buffComp)) {
            if (buffComp.StatusEffects.ContainsKey("grace_period")) {
                this.debugMessage = "I have Grace Period.";
                return false;
            }
        }
        else {
            this.debugMessage = "Can not find my Buffs component.";
            return false;
        }

        if (!ui.me.GetComp<Actor>(out var _)) {
            this.debugMessage = "Can not find my Actor component.";
            return false;
        }
